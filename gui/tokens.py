...
# ── §4 Spacing ───────────────────────────────────────────────────
# §4's ratified values are 14 / 8 / 16, sized against ariana's display. On
# tanzania's 1366x768 panel the dashboard's natural height overruns the usable
# work area, and spacing is the right thing to give up first -- it costs
# density, not information. Every panel, row, and control is still present.
PANEL_PAD   = 9        # panel padding (§4: 14)
GAP_INTERNAL = 5       # internal element gap (§4: 8)
GAP_REGION  = 10       # between major regions (§4: 16)

# ── §3 Typography ────────────────────────────────────────────────
# §3's stack is `-apple-system, ..., "SF Pro Text", ...`. On macOS that resolves
# to SF Pro Text, reached via Qt's ".AppleSystemUIFont" alias -- Linux has no
# such alias. Qt Style Sheets accept a comma-separated fallback list, same as
# CSS, so this ports as an actual fallback chain instead of a single name:
# "Inter" (design-adjacent to SF Pro, common on Linux desktops when installed),
# then the desktop's own UI sans (GNOME/Cantarell, otherwise Noto/DejaVu Sans),
# with sans-serif as the final generic fallback so this never resolves to
# nothing on a minimal system.
FAMILY = '"Inter", "Cantarell", "Noto Sans", "DejaVu Sans", sans-serif'
