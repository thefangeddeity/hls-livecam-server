...
Decode is software (no `-hwaccel`). At 640x360/15fps this is trivially cheap --
well under a percent of a core measured against tanzania's own encode pipeline
budget -- so there is no need to guess at which hwaccel API (vaapi/nvdec/qsv)
this box's GPU stack actually supports. If preview CPU cost ever becomes a real
line item, that is worth measuring properly rather than assumed here.
...
class VideoWorker(QThread):
    ...
    def _spawn(self):
        return subprocess.Popen(
            ...
            # No visible window or terminal artifact. A pipe-attached child has
            # no controlling terminal of its own already; start_new_session also
