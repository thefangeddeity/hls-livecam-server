...
Phase 1 scope: READ-ONLY. It taps the RTSP loopback the same way
video_preview.rs does, runs the detector, and prints state. It never
writes to the stream, never touches the capture pipeline, and nothing
downstream depends on it -- if this process dies the node keeps
publishing exactly as before.

Deliberately NOT running CVProcessor. That class is the picture-
enhancement pipeline (optical flow, CLAHE, denoise, sharpen) measured at
121.7 ms/frame against a 66.7 ms budget on a node stronger than tina;
its cost buys a processed *picture*, which is Phase 2. Detection alone is
the detector plus the tracker, and it can run at a few frames a second
because "what is in the room" does not change at 15 Hz.
