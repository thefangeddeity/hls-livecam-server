...
The source is two cats peeking through a white circular opening on a
deep-blue field, with a wide blue margin and the opening sitting
off-centre. A straight downscale would leave the opening a small
off-centre blob at 16px, so the small sizes (16/32) get a TIGHT crop to
the opening; the larger sizes (48/256) keep more of the blue composition.
Crops are centred on the opening's measured bounding box (the
non-background content), not guessed pixels, so this re-runs
deterministically if the source is ever replaced.
